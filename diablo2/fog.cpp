...
bool fog::mpq_open_file(char* path, structures::file_handle** outHandle) {
  static wrap_func_fast_import<BOOL(char*, structures::file_handle**)>
      mpq_open_file(10102, get_base());
  return mpq_open_file(path, outHandle);
}

bool fog::mpq_close_file(structures::file_handle* handle) {
  static wrap_func_fast_import<BOOL(structures::file_handle*)> mpq_close_file(
      10103, get_base());
  return mpq_close_file(handle);
}

bool fog::mpq_read_file(structures::file_handle* handle,
                        void* buffer,
                        size_t size,
                        size_t* bytesToRead) {
  static wrap_func_fast_import<BOOL(structures::file_handle*,
                                    void*,
                                    size_t,
                                    size_t*,
                                    int32_t,
...
size_t fog::mpq_get_file_size(structures::file_handle* handle, size_t* compressedSize) {
  static wrap_func_fast_import<size_t(structures::file_handle*, size_t*)>
