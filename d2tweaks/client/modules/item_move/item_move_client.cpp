...
int32_t __fastcall item_click(unit* owner, inventory* inventory, int mouse_x, int mouse_y, uint8_t flag, char* a6, unsigned int page) {
  ...
  const auto currentInventoryIndex = d2_common::get_inventory_index(player, page, d2_client::is_lod());
  const auto clickedItem = d2_common::get_item_at_cell(player->inventory, itemx, itemy, &px, &py, currentInventoryIndex, page);
...
void item_move::init() {
  ...
  if (GetPrivateProfileInt("modules", "ItemMover", 1, acPathToIni) != FALSE) {
    hooking::hook(d2_client::get_base() + 0x475C0, item_click, reinterpret_cast<void**>(&g_item_click_original));
...
void item_move::handle_packet(common::packet_header* packet) {
  ...
  const auto inventoryIndex = d2_common::get_inventory_index(player, itemMove->target_page, d2_client::is_lod());

  item->item_data->page = itemMove->target_page;

  d2_common::inv_add_item(player->inventory, item, itemMove->tx, itemMove->ty, inventoryIndex, true, item->item_data->page);
