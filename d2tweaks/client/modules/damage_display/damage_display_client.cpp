...
struct damage_label {
  ...
  damage_label(uint32_t x, uint32_t y, uint32_t uw, uint32_t uh, int32_t damage, int32_t hp, int32_t maxHp)
    : screen_space(false), color(ui_color_t::UI_COLOR_WHITE), x(x), y(y),
  ...
  damage_label()
    : screen_space(false), color(UI_COLOR_WHITE), x(0), y(0),
  ...
  for (size_t i = 0; i < DAMAGE_LABELS_SIZE; i++) {
    ...
    if (label->screen_space) {
      ...
      if (label->currentHp > 0) {
        ...
        textColor = healthPercentage >= 0.8f ? UI_COLOR_GREEN : (healthPercentage <= 0.5f ? UI_COLOR_RED : UI_COLOR_YELLOW);
        ...
        int32_t combinedTextWidth = d2_win::get_text_pixel_width(const_cast<wchar_t*>(combinedTextPtr));
        ...
        d2_win::draw_text(const_cast<wchar_t*>(combinedText.c_str()), textX, textY, textColor, 0);
        d2_gfx::draw_filled_rect(textX, textY, textX + static_cast<int>(healthPercentage * combinedTextWidth), textY + _barHeight, barColor, 255);
        ...
        d2_win::draw_text(const_cast<wchar_t*>(dmgTextPtr), textX + d2_win::get_text_pixel_width(const_cast<wchar_t*>(combinedTextPtr)) / 2, my + label->unit_height / 2, textColor, 0);
      }
    }
  }
...
void damage_display::init() {
  ...
  if (GetPrivateProfileInt("modules", "DamageDisplay", 1, acPathToIni) != FALSE) {
    ...
    hooking::hook(d2_client::get_base() + 0x80A30, draw_game_ui, reinterpret_cast<void**>(&g_draw_game_ui));
  }
}
...
void damage_display::handle_packet(common::packet_header* packet) {
  ...
  int32_t index;
  if (d2_client::cache_gfx_data(&gfxdata, monsterUnit, nullptr, 0, 0, &index, 1, -1) &&
