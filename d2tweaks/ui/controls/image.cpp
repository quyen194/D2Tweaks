...
void image::draw(int32_t offsetX, int32_t offsetY) {
  ...
  m_draw_info.cell_file =
      static_cast<cellfile*>(m_image->get());
  m_draw_info.frame = m_frame;

  d2_gfx::draw_image(
      &m_draw_info, get_x() + offsetX, get_y() + offsetY, -1, 5, nullptr);
}

void image::left_mouse(int32_t offsetX, int32_t offsetY, bool up, bool& block) {
  if (!m_block_click)
    return;

  block = m_rect.contains(d2_client::get_mouse_x(),
                          d2_client::get_mouse_y(),
                          offsetX,
                          offsetY);
}

void image::right_mouse(int32_t offsetX,
                        int32_t offsetY,
                        bool up,
                        bool& block) {
  if (!m_block_click)
