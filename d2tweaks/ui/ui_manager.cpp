...
static steady_clock::time_point lastToggleTime;
...
void sendPacketAndUpdateProperty(int gemBagGuid,
                                 uint32_t iCode,
                                 int prop,
                                 int val,
                                 int item_guid,
  ...
  if (250 < GetTickCount() - D2CLIENT_StoredTickCount1) {
    ...
    d2_common::add_property(gemBag, &itemProperty, 0);
  }
}
...
LRESULT ui_manager::wnd_proc(HWND hWnd,
  ...
  if (wParam == 'Z' || wParam == 'G') {
    ...
    if (g_hoverItem != 0) {
      const auto record =
          d2_common::get_item_record(g_hoverItem->data_record_index);
  ...
  switch (msg) {
    ...
    case WM_RBUTTONDOWN: {
      ...
      if (g_hoverItem != nullptr) {
        ...
        const auto record = d2_common::get_item_record(g_hoverItem->data_record_index);
        ...
        auto currentPage = d2_common::get_item_page(g_hoverItem);
        ...
        if (currentPage == 0 || currentPage == 4) {
          ...
          if (d2_client::get_ui_window_state(UI_WINDOW_STASH) ||
            d2_client::get_ui_window_state(UI_WINDOW_INVENTORY)) {
          ...
          else if (d2_client::get_ui_window_state(UI_WINDOW_CUBE) && currentPage == 3) {
            // Iterate through each gem type in the gemTypes map
            for (const auto& gem : gemTypes) {
              // Accessing the key and value of the gemTypes map
              const std::string& _key = gem.first;
          ...
          else if (d2_client::get_ui_window_state(UI_WINDOW_CUBE) && currentPage == 0) {
            // Iterate through each gem type in the gemTypes map
            for (const auto& gem : gemTypes) {
              // Accessing the key and value of the gemTypes map
              const std::string& _key = gem.first;
          ...
          if (d2_client::get_ui_window_state(UI_WINDOW_STASH) ||
            ...
            for (const auto& gem : exTypes) {
              ...
              if (strncmp(normCode, key, 3) == 0) {
                ...
                d2_client::send_to_server(&D2GSPacketClt20, sizeof D2GSPacketClt20);
        ...
        if (isGemOrRuneCode(normCode)) {
          ...
          for (auto item = player->inventory->first_item; item != nullptr; item = item->item_data->pt_next_item) {
            ...
            currentPage = d2_common::get_item_page(item);
        ...
        */
        ...
        if (strncmp(normCode, "ib1", 3) == 0) {
          ...
          auto& toggle_menu =
              singleton<loot_filter_settings_toggle_menu>::instance();
    ...
    case WM_MBUTTONDOWN: {
      ...
      // Calculate the time elapsed since the last stash toggle
      auto timeElapsed = duration_cast<milliseconds>(currentTime - lastToggleTime).count();
