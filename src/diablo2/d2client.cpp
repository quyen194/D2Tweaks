...
void diablo2::d2_client::print_chat(wchar_t* string, uint32_t color) {
	static wrap_func_fast <void(wchar_t*, uint32_t)>print_chat(0x7C600, get_base());
...
void diablo2::d2_client::play_sound(const uint32_t soundId, structures::unit* u, const uint32_t ticks, const BOOL prePick, const uint32_t cache) {
	static wrap_func_fast<void(uint32_t, structures::unit*, uint32_t, BOOL, uint32_t)>play_sound(0xB5820, get_base());
...
uint32_t diablo2::d2_client::screen_width() {
	return *reinterpret_cast<uint32_t*>(get_base() + 0xD40F4);
}

uint32_t diablo2::d2_client::current_vendor_id() {
	return *reinterpret_cast<uint32_t*>(get_base() + 0x115CFD);
}

uint32_t diablo2::d2_client::current_vendor_guid() {
	return *reinterpret_cast<uint32_t*>(get_base() + 0x115CF5);
}

bool diablo2::d2_client::is_gamble_open() {
