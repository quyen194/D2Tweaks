...
// Get a profile string value, if the buffer size is not large enough, the
...
BOOL CIni::WriteString(LPCTSTR lpSection,
  ...
  return ::WritePrivateProfileString(
      lpSection, lpKey, lpValue == NULL ? _T("") : lpValue, m_pszPathName);
}

// Read a string value from the ini file, append another string after it and
// then write it back to the ini file
BOOL CIni::AppendString(LPCTSTR lpSection,
                        LPCTSTR lpKey,
...
DWORD CIni::GetArray(LPCTSTR lpSection,
  ...
  if (*psz != _T('\0')) {
    if (lpBuffer == NULL) {
      ...
      dwCopied =
          __StringSplit(psz, lpBuffer, dwBufSize, lpDelimiter, bTrimString);
    }
  }
...
#ifdef __AFXWIN_H__
void CIni::GetArray(LPCTSTR lpSection,
                    LPCTSTR lpKey,
                    CStringArray* pArray,
                    LPCTSTR lpDelimiter,
                    BOOL bTrimString) const {
...
#ifdef __AFXWIN_H__
BOOL CIni::WriteArray(LPCTSTR lpSection,
                      LPCTSTR lpKey,
                      const CStringArray* pArray,
                      int nWriteCount,
  ...
  const CString DELIMITER =
      (lpDelimiter == NULL || *lpDelimiter == _T('\0')) ? _T(",") : lpDelimiter;
  CString sLine;
...
int CIni::GetInt(LPCTSTR lpSection,
  ...
  return *sz == _T('\0') ? nDefault
                         : int(_tcstoul(sz, NULL, __ValidateBase(nBase)));
}
...
UINT CIni::GetUInt(LPCTSTR lpSection,
  ...
  GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
  return *sz == _T('\0') ? nDefault
                         : UINT(_tcstoul(sz, NULL, __ValidateBase(nBase)));
}
...
// Read a boolean value from the ini file, invert it(true becomes false, false
// becomes true), then write it back
BOOL CIni::InvertBool(LPCTSTR lpSection, LPCTSTR lpKey) const {
  return WriteBool(lpSection, lpKey, !GetBool(lpSection, lpKey, FALSE));
}

// Read a int from the ini file, increase it and then write it back to the ini
// file
...
// Read an UINT from the ini file, increase it and then write it back to the ini
// file
...
DWORD CIni::GetKeyLines(LPCTSTR lpSection,
  ...
  if (lpBuffer == NULL) {
    ...
    DWORD dwCopied =
        ::GetPrivateProfileSection(lpSection, psz, dwLen, m_pszPathName);
    ...
    return ::GetPrivateProfileSection(
        lpSection, lpBuffer, dwBufSize, m_pszPathName);
  }
}
...
BOOL CIni::CopySection(LPCTSTR lpSrcSection,
  ...
  const BOOL RES =
      ::WritePrivateProfileSection(lpDestSection, psz, m_pszPathName);
  delete[] psz;
...
BOOL CIni::MoveKey(LPCTSTR lpSrcSection,
                   ...
                   BOOL bFailIfExist) const {
  return CopyKey(
             lpSrcSection, lpSrcKey, lpDestSection, lpDestKey, bFailIfExist) &&
...
LPTSTR CIni::__GetStringDynamic(LPCTSTR lpSection,
  ...
  psz = new TCHAR[dwLen + 1];
  DWORD dwCopied =
      ::GetPrivateProfileString(lpSection,
                                lpKey,
  ...
  while (dwCopied + 1 >= dwLen) {
    ...
    delete[] psz;
    psz = new TCHAR[dwLen + 1];
    dwCopied = ::GetPrivateProfileString(lpSection,
                                         lpKey,
                                         lpDefault == NULL ? _T("") : lpDefault,
                                         psz,
                                         dwLen,
...
// Split a string usinf a particular delimiter, split result are copied into
// lpBuffer in the "double null terminated string" format as the following
// figure shows: xxx\0xxxx\0xx\0xxx\0\0
//
...
DWORD CIni::__StringSplit(LPCTSTR lpString,
  ...
  *lpBuffer = _T('\0');
  ...
  LPTSTR pszDel = (lpDelimiter == NULL) ? _tcsdup(DEF_PROFILE_DELIMITER)
                                        : _tcsdup(lpDelimiter);
...
// Parse a "double null terminated string", pass each sub string to a
