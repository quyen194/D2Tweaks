...
static void* allocate_function_stub(void* origin, void* ptr, size_t size) {
  ...
  if (!current_stub) {
    current_stub = VirtualAlloc(nullptr,
                                MEMORY_BLOCK_SIZE,
  ...
  *reinterpret_cast<uint16_t*>(code + 5) = 0xE0FF;
  ...
  current_stub =
      reinterpret_cast<void*>(reinterpret_cast<uint64_t>(current_stub) + size);
...
hooking::mh_status_t hooking::details::hook(void* target,
                                            ...
                                            void** original) {
  mh_status_t result =
      static_cast<mh_status_t>(MH_CreateHook(target, detour, original));
...
void* hooking::set_call(void* address, void* function, size_t stubSize) {
  ...
  *reinterpret_cast<int32_t*>(static_cast<char*>(address) + 1) =
      stub - reinterpret_cast<int32_t>(address) - 5;
